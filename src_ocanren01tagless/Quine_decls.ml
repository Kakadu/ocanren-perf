...
module Triple = struct
  include MiniKanrenCore.Fmap3(struct
    type ('a,'b,'c) t = 'a * 'b * 'c
    let fmap f g h (a,b,c) = (f a, g b, h c)
  end)

  let triple x y z = inj @@ distrib (x,y,z)
end
...
module Gterm = struct
  module X = struct
    type ('s, 'xs) t =
      | Symb  of 's
      | Seq   of 'xs [@@deriving gt ~options:{show}]
    ...
    let t = {
      ...
      plugins = object
        ...
        method show fa fb bx =
           GT.transform(t)
              (fun fself -> object inherit ['a,'b,_] show_t_t (GT.lift fa) (GT.lift fb) fself
...
let rec gterm_reifier c : Gterm.fterm -> Gterm.lterm =
  Gterm.reify reify (List.reify gterm_reifier) c
...
let rec gresult_reifier c : Gresult.fresult -> Gresult.lresult =
  Gresult.reify reify gterm_reifier
    (List.reify (Pair.reify reify gresult_reifier))
    c
...
and evalo (term: fterm) (env: fenv) (r: fresult) =
  ...
  ; fresh (func arge arg x body env')
      ...
      (evalo body ((Pair.pair x arg) % env') r)
...
let wrap3terms t =
  t#reify
    (Triple.reify gterm_reifier gterm_reifier gterm_reifier)
  |> (function
      | Var _ -> assert false
      | Value (a,b,c) ->
          printf "* %s\n  %s\n  %s\n\n"
