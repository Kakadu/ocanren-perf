...
let wrap : ?n:int -> string * (Numero_decls_trace.ioleg -> MiniKanren.goal) -> unit =
  fun ?(n = 1) (msg, goal) ->
  print_endline msg;
  MiniKanren.(run q) goal
    (fun s  -> MiniKanren.Stream.take ~n s |>
      List.iteri (fun i r ->
        let n = r#refine
          Numero_decls_trace.num_reifier
          ~inj:(MiniKanrenStd.List.to_logic (fun x -> MiniKanren.Value x))
        in
