...
module Gterm = struct
  ...
  let symb n : injected = OCanren.inj @@ distrib (Symb n)
  let vr n : injected = OCanren.inj @@ distrib (VR n)
  let tuple xs : injected = inj @@ distrib (Tuple (Std.list Fun.id xs))

  let (!!) x = inj (lift x)
  let quote xs = inj @@ distrib (Tuple (Std.List.cons (symb !!"quote")  xs))
  let quotequote : injected = quote (Std.List.(!<) (symb !!"quote"))
  ...
  let app func arg = inj @@ distrib (Tuple (Std.List.(%<)  func  arg))
  let list xs : injected = inj @@ distrib (Tuple (Std.List.cons (symb !!"list") xs))
  let list2 a b : injected = inj @@ distrib (Tuple (Std.List.cons (symb !!"list") (Std.List.(%<) a   b)))
  ...
  let reify: (ground, logic) Reifier.t =
    ...
    Reifier.fix (fun self ->
      Reifier.reify <..>
        chain (Reifier.zed (Reifier.rework ~fv:(fmapt OCanren.reify OCanren.Std.Nat.reify (Std.List.reify self)))))
...
module Gresult = struct
  ...
  type nonrec 'env ground = ('env, Var.ground, Gterm.ground) t [@@deriving gt ~options:{ fmt }]
  type nonrec 'env logic = ('env, Var.logic, Gterm.logic) t OCanren.logic [@@deriving gt ~options:{ fmt }]
  ...
  type env_injected = ((Var.ground, rresult) Std.Pair.ground , (Var.logic, lresult) Std.Pair.logic) Std.List.injected
  type nonrec injected = (rresult, lresult) OCanren.injected


  let closure a b c : injected = OCanren.inj @@ distrib (Closure (a,b,c))
  let code  a : injected = OCanren.inj @@ distrib (Code a)
  let clo = closure
...
IFDEF TRACE THEN
 ...
 let ( ===!! ) : _ Std.List.injected -> _ Std.List.injected -> goal =
   fun x y st ->
    ...
    OCanren.( === ) x y st
   [@@inline]
 let ( ==== ) : (string, string OCanren.logic) OCanren.injected as 'a -> 'a -> goal =
   fun x y st ->
    incr_counter ();
    OCanren.( === ) x y st
   [@@inline]
...
let rec vl : Gresult.injected -> goal = fun o ->
  conde
