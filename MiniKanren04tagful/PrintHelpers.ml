module StringLo = struct
  ...
  type injected = (GT.string, string OCanren.logic) OCanren.injected
...
module ListLo = struct
  ...
  type ('a, 'b) injected = ('a, 'b) Std.List.injected

  let prj_exn = Std.List.prj_exn
  let reify = Std.List.reify
  let project f xs = Std.List.logic_to_ground_exn f xs
end
